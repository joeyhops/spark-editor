...
module.exports = {
  ...
  loadBoids: function() {
    var boids = Path.join('mode_assets', 'telus', 'boids');
    var tempProject = Path.join(os.tmpdir(), 'telus' + Date.now(), 'Boids');
    wrench.mkdirSyncRecursive(tempProject);
    wrench.copyDirSyncRecursive(boids, tempProject, {
      excludeHiddenUnix: true,
      inflateSymlinks: true,
      forceDelete: true
    });
    ...
    var self = this;
    this.loadProject(tempProject, function(){
      self.openFile(Path.join(tempProject, 'Coding.js'));
      gui.Window.get().show();
    });
  },
  ...
  resetBoids: function () {
    ...
    var self = this;
    this.resetProject(tempProject, true);
  },
  ...
  run: function() {
    ...
    if (this.outputWindow) {
      ...
      console.log('start server', this.projectPath);
